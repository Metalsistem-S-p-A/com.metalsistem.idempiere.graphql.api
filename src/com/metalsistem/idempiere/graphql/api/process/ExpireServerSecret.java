...
package com.metalsistem.idempiere.graphql.api.process;
...
import com.metalsistem.idempiere.graphql.api.auth.SysConfigTokenSecretProvider;
...
public class ExpireServerSecret extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		DB.executeUpdateEx("UPDATE AD_SysConfig SET Value = ? WHERE AD_Client_ID = ? AND AD_Org_ID = ? AND Name = ?",
				new Object[] { UUID.randomUUID().toString(), 0, 0, SysConfigTokenSecretProvider.GRAPHQL_TOKEN_SECRET },
				get_TrxName());
		CacheMgt.get().reset(MSysConfig.Table_Name);
		
		DB.executeUpdateEx("UPDATE GraphQL_AuthToken SET IsActive = 'N', IsExpired = 'Y'", get_TrxName());
		CacheMgt.get().reset(MGraphQLAuthToken.Table_Name);
