 ...
 *
 */
public class MGraphQLAuthToken extends X_GraphQL_AuthToken implements ImmutablePOSupport {
	/**
	 * 
	 */
	...
	private static ImmutablePOCache<String, MGraphQLAuthToken> s_authtoken_cache = new ImmutablePOCache<String, MGraphQLAuthToken>(Table_Name, 40);
	
	public MGraphQLAuthToken(Properties ctx, int GraphQL_AuthToken_ID, String trxName) {
		super(ctx, GraphQL_AuthToken_ID, trxName);
	}

	public MGraphQLAuthToken(Properties ctx, String GraphQL_AuthToken_UUID, String trxName) {
		super(ctx, GraphQL_AuthToken_UUID, trxName);
	}

	public MGraphQLAuthToken(Properties ctx, ResultSet rs, String trxName) {
	 ...
	 * @param copy
	 */
	public MGraphQLAuthToken(MGraphQLAuthToken copy) {
	 ...
	 * @param copy
	 */
	public MGraphQLAuthToken(Properties ctx, MGraphQLAuthToken copy) {
	 ...
	 * @param trxName
	 */
	public MGraphQLAuthToken(Properties ctx, MGraphQLAuthToken copy, String trxName) {
		this(ctx, 0, trxName);
		copyPO(copy);
	}
	 ...
	 */
	public static MGraphQLAuthToken get(Properties ctx, String token) {
		MGraphQLAuthToken retValue = null;
		if (s_authtoken_cache.containsKey(token)) {
			retValue = s_authtoken_cache.get(ctx, token.toString(), e -> new MGraphQLAuthToken(ctx, e));
			return retValue;
		}
		retValue = new Query(ctx, MGraphQLAuthToken.Table_Name, "Token=?", null)
				.setParameters(token)
				.first();
		s_authtoken_cache.put(token, retValue, e -> new MGraphQLAuthToken(ctx, e));
		return retValue;
	}

	public static boolean isBlocked(String token) {
		MGraphQLAuthToken auth = get(Env.getCtx(), token);
	...
	public static int deactivateTokens(int userId, int roleId, int clientId, String trxName) {
		...
		List<MGraphQLAuthToken> list = query.list();
		int cnt = 0;
		for(MGraphQLAuthToken authToken : list) {
