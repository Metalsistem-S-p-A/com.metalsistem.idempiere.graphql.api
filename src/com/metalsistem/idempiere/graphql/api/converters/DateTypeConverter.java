...
* - Heng Sin Low                                                      *
**********************************************************************/
package com.metalsistem.idempiere.graphql.api.converters;
	...
	private Timestamp fromJsonValue(int displayType, JsonElement value) {
		...
		if (DisplayType.isDate(displayType) && pattern != null && value != null) {
			...
			try {
				...
				throw new IDempiereGraphQLException("Invalid date", "The " + DisplayType.getDescription(displayType) + " pattern should be: " + pattern + ". Exception: " + e.getLocalizedMessage(), Status.BAD_REQUEST);
			}
